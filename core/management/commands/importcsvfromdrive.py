                ...
                try:
                    ...
                    if row["Aged"]:
                        if row["Aged"].isdigit():
                            ...
                            self.stdout.write(self.style.WARNING('{} Could not parse age as years "{}"'.format(row["Import Key"], row["Aged"])))

                    if row["Died"]:
                        try:
                            dod = datetime.datetime.strptime(row["Died"], "%d.%m.%Y").date()
                            name.date_of_death = dod
                        except ValueError:
                            self.stdout.write(self.style.WARNING('{} Could not parse date of death "{}"'.format(row["Import Key"], row["Died"])))

                    if row["Born"]:
                        try:
                            dob = datetime.datetime.strptime(row["Born"], "%d.%m.%Y").date()
                            name.date_of_birth = dob
                        except ValueError:
                            self.stdout.write(self.style.WARNING('{} Could not parse date of birth "{}"'.format(row["Import Key"], row["Born"])))
                    ...
                    self.stdout.write(self.style.ERROR('Unable to find location {}'.format(row["Section"])))
